...
export function Testimonial(props: TestimonialProps) {
  const { handle, image, name, position, text } = props;
  ...
  return (
    <div className="text-center px-12 py-8 pt-20 mx-4 md:mx-0">
      ...
      <blockquote className="text-xl font-medium mb-4">{text}</blockquote>
      <cite className="block font-bold text-lg not-italic mb-1">{name}</cite>
      <div className="text-gray-600">
        <span>{position}</span>{" "}
        <a title="adress" className="text-blue-600 hover:underline" href="#0">
          @{handle}
        </a>
      </div>
    </div>
